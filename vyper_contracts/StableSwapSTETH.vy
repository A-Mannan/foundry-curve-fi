...
def get_D(xp: uint256[N_COINS], amp: uint256) -> uint256:
    ...
    for _i in range(255):
        D_P: uint256 = D
        for _x in xp:
            D_P = D_P * D / (_x * convert(N_COINS, uint256) + 1)  # +1 is to prevent /0
        Dprev = D
        D = (Ann * S / A_PRECISION + D_P * convert(N_COINS, uint256)) * D / ((Ann - A_PRECISION) * D / A_PRECISION + (N_COINS + 1) * D_P)
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256) -> uint256:
    ...
    if token_supply > 0:
        ...
        fee: uint256 = self.fee * convert(N_COINS, uint256) / (4 * (N_COINS - 1))
...
def get_y(i: int128, j: int128, x: uint256, xp: uint256[N_COINS]) -> uint256:
    ...
    for _i in range(N_COINS):
        if _i == i:
            ...
            continue
        S_ += _x
        c = c * D / (_x * convert(N_COINS, uint256))
    c = c * D * A_PRECISION / (Ann * convert(N_COINS, uint256))
...
def remove_liquidity_imbalance(
    ...
    fees: uint256[N_COINS] = empty(uint256[N_COINS])
    fee: uint256 = self.fee * convert(N_COINS,uint256) / (4 * (convert(N_COINS, uint256) - 1))
