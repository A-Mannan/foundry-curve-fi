...
def unpack_2(packed: uint256) -> uint256[2]:
    ...
    return [packed & (2**128 - 1), packed /(2** 128)]
...
def _calc_moving_average(
    ...
    last_ema_value: uint256 = (packed_value / (2**128))
...
def ema_price(i: uint256) -> uint256:
    ...
    return (self.last_prices_packed[i] /(2**128))
...
def exp(x: int256) -> uint256:
    ...
    y: int256 = unsafe_add(unsafe_mul(unsafe_add(value, 1346386616545796478920950773328), value) /(2** 96), 57155421227552351082224309758442)
    p: int256 = unsafe_add(unsafe_mul(unsafe_add(unsafe_mul(unsafe_sub(unsafe_add(y, value), 94201549194550492254356042504812), y) /(2** 96),\
                           28719021644029726153956944680412240), value), 4385272521454847904659076985693276 * (2**96))
    ...
    q: int256 = unsafe_add(unsafe_mul(unsafe_sub(value, 2855989394907223263936484059900), value) /(2** 96), 50020603652535783019961831881945)
    q = unsafe_sub(unsafe_mul(q, value) /(2** 96), 533845033583426703283633433725380)
    q = unsafe_add(unsafe_mul(q, value) /(2** 96), 3604857256930695427073651918091429)
    q = unsafe_sub(unsafe_mul(q, value) /(2** 96), 14423608567350463180887372962807573)
    q = unsafe_add(unsafe_mul(q, value) /(2** 96), 26449188498355588339934803723976023)
    ...
    return unsafe_mul(convert(convert(r, bytes32), uint256), 3822833074963236453042738258902158003155416615667)/(2** convert(unsafe_sub(195, k), uint256))
